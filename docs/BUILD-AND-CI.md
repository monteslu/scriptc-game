...
| Job | Runner | What |
| --- | --- | --- |
| `linux-x86_64` | ubuntu-24.04 | Build every example, run every native test and the browser smoke |
...
The browser smoke runs `station` on every platform. It covers the full game
stack in one pass: assets, input, audio, animation, Canvas HUD work, and
WebGL. Native builds and native tests are not reduced.
