/* WebGL2 constants, generated from webgl-node/lib/constants.mjs.
 *
 * On the web these are properties of the context (gl.TEXTURE_2D), assigned
 * in a loop over the constant table. That loop cannot work here: the dialect
 * has no dynamic property assignment, and 519 fields on one class would be
 * 519 field reads through a receiver besides.
 *
 * They are module constants instead, so a game writes:
 *
 *     import { TEXTURE_2D } from ".../constants.js";
 *     gl.bindTexture(TEXTURE_2D, tex);
 *
 * That is the ONE deviation from gl.TEXTURE_2D, and it is the same trade the
 * Gamepad layer makes: the values are identical, only the spelling differs.
 */
...
export const RENDERBUFFER_OBJ = 0x8D41;
